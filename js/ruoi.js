...
function Fly(fid, type, color, sh, sw, x, y, fly_img, name, title) {
    ...
    if (this.name == 'Ruồi nhút nhát') {
        $(t).html(this.name + " nói:\n" + '"Chủ nhân ta nói nhớ Bảo Ngọc nhiều lắm hehe"');
        this.gender = "Đực";
    } else if (this.name == 'Ruồi cool ngầu') {
        $(t).html(this.name + " nói:\n" + '"Ta có một bí mật, chủ nhân ta thương ngươi lắm đó"');
        this.gender = "Đực";
    } else if (this.name == 'Ruồi lãng tử') {
        $(t).html(this.name + " nói:\n" + '"Ta ghét ngươi sô mớt"');
        this.gender = "Cái";
    } else {
        $(t).html(this.name + " nói:\n" + '"Ngươi biết không, ngươi thật là đẹp"');
        this.gender = "Đực";
    }
        ...
        setTimeout(function() {
            ...
            switch (value_message) {
                ...
                case 3:
                    ...
                    break;
                case 4:
                    $(t).html(parent.name + " nói:\n" + '"Ngươi lạnh lùng mà sao lại tàn sát ta, ta....vẫn thương ngươi"')
                    break;
                case 5:
                    $(t).html(parent.name + " nói:\n" + '"Ngươi dám đánh ta à ta về méc chủ nhân ta, không dễ bỏ qua đâu"')
                    break;
                case 6:
                    $(t).html(parent.name + " nói:\n" + '"Giết ruồi đền mạng nhưng vì ngươi là ghệ của ông chủ ta tha đó"')
                    break;
                case 7:
                    $(t).html(parent.name + " nói:\n" + '"Ngươi có biết đau đớn nhất trên cuộc đời này là gì không ? Là nhớ một người"')
                    break;
                case 8:
                    $(t).html(parent.name + " nói:\n" + '"Cuộc sống này thật buồn khi vắng ngươi haizzz"')
                    break;
                case 9:
                    $(t).html(parent.name + " nói:\n" + '"Ngươi có biết rắng vì ngươi mà ta tăng mấy độ cận không *-*, ta đùa thôi"')
                    break;
                case 10:
                    $(t).html(parent.name + " nói:\n" + '"Ngươi giết ta nhưng không thể làm ta ngưng nhớ ngươi được"')
                    break;
            }
            if (parent.hasclick) {
                $(t).fadeIn(200);
